...
This file is craped from the tutorial it may or may not contain the correct code
please be aware of you using the right code.
"""
...
def load_model(training_config, load_base_model=False):
    ...
    model = transformers.AutoModelForCausalLM.from_pretrained(model_load_path)
    tokenizer = transformers.AutoTokenizer.from_pretrained(model_load_path)
...
class Trainer(transformers.Trainer):
    ...
    def update_log_timing(self, logs):
        ...
        logs["iter_time"] = round(iter_time, 2)
        self.start_time = time.time()

        logs["flops"] = int(self.model_flops)
        logs["tflops"] = (
            round((self.model_flops * self.total_steps) / (iter_time * 10 ** 12), 4)
            if iter_time > 0
            else float("nan")
        )
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        # Data loader must be populated before this method
        return transformers.Trainer.evaluate(self, eval_dataset, ignore_keys, metric_key_prefix)

# HF trainer
def start_trainer(training_config, model, tokenizer, device, train_dataset, test_dataset):
    ...
    model_flops = model.config.n_positions * model.config.n_embd * model.config.n_layer
    print("Start training, model FLOPS = ", model_flops)

    model_name = training_config["model"]["pretrained_name"]
    total_steps = training_config["train"]["total_steps"]
    logging_steps = training_config["train"]["logging_steps"]
    ...
    training_args = transformers.TrainingArguments(
        ...
        save_steps=training_config["train"]["save_steps"],
        save_total_limit=1,
        evaluation_strategy="steps",
        eval_steps=training_config["train"]["eval_steps"],
        prediction_loss_only=True,
        learning_rate=training_config["train"]["learning_rate"],
        optim="adamw_torch",
